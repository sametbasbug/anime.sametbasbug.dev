...
Yerel yapılandırma için `.env.example` dosyasını `.env` olarak kopyala ve Supabase publishable değerlerini ekle. Migration ile güvenlik ayrıntıları [`docs/ACCOUNT_ARCHITECTURE.md`](./docs/ACCOUNT_ARCHITECTURE.md) içinde belgelenmiştir. Geliştirme projesi Supabase Free üzerinde kurulmuştur; ortam değerleri yoksa uygulama güvenli biçimde yerel modda kalır. Üretimde Supabase Auth URL'leri ve Resend özel SMTP, `Rota <giris@sametbasbug.dev>` göndericisiyle yapılandırılmıştır.
...
4. **Hesap ve kalıcı veri** — Supabase ve özel SMTP üretimde çalışıyor, fiziksel cihaz doğrulaması bekliyor
